...
#' @description InventoryCount Class
...
InventoryCount <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new InventoryCount class.
    #'
    ...
    #'
    #' @return InventoryCount as a base R list.
    #' @examples
    #' # convert array of InventoryCount (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert InventoryCount to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      InventoryCountObject <- list()
      if (!is.null(self$`countDate`)) {
        InventoryCountObject[["countDate"]] <-
          self$`countDate`
      }
      if (!is.null(self$`countNumber`)) {
        InventoryCountObject[["countNumber"]] <-
          self$`countNumber`
      }
      if (!is.null(self$`lineItems`)) {
        InventoryCountObject[["lineItems"]] <-
          self$extractSimpleType(self$`lineItems`)
      }
      if (!is.null(self$`notes`)) {
        InventoryCountObject[["notes"]] <-
          self$`notes`
      }
      if (!is.null(self$`status`)) {
        InventoryCountObject[["status"]] <-
          self$extractSimpleType(self$`status`)
      }
      if (!is.null(self$`warehouseId`)) {
        InventoryCountObject[["warehouseId"]] <-
          self$`warehouseId`
      }
      return(InventoryCountObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of InventoryCount
    #'
    #' @param input_json the JSON input
    #' @return the instance of InventoryCount
    ...
    #' @return InventoryCount in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to InventoryCount and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`countDate`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for InventoryCount: the required field `countDate` is missing."))
      }
      ...
      if (!is.null(input_json$`countNumber`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for InventoryCount: the required field `countNumber` is missing."))
      }
      # check the required field `lineItems`
      if (!is.null(input_json$`lineItems`)) {
        stopifnot(R6::is.R6(input_json$`lineItems`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for InventoryCount: the required field `lineItems` is missing."))
      }
      # check the required field `status`
      if (!is.null(input_json$`status`)) {
        stopifnot(R6::is.R6(input_json$`status`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for InventoryCount: the required field `status` is missing."))
      }
      ...
      if (!is.null(input_json$`warehouseId`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for InventoryCount: the required field `warehouseId` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of InventoryCount
    toString = function() {
      self$toJSONString()
    },
...
# InventoryCount$unlock()
#
## Below is an example to define the print function
# InventoryCount$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
