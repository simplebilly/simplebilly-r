#' Create a new CustomerCommunication
#'
#' @description
...
#' @description CustomerCommunication Class
...
#' @export
CustomerCommunication <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new CustomerCommunication class.
    #'
    ...
    #'
    #' @return CustomerCommunication as a base R list.
    #' @examples
    #' # convert array of CustomerCommunication (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CustomerCommunication to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      CustomerCommunicationObject <- list()
      if (!is.null(self$`body`)) {
        CustomerCommunicationObject[["body"]] <-
          self$`body`
      }
      if (!is.null(self$`channel`)) {
        CustomerCommunicationObject[["channel"]] <-
          self$extractSimpleType(self$`channel`)
      }
      if (!is.null(self$`contactId`)) {
        CustomerCommunicationObject[["contactId"]] <-
          self$`contactId`
      }
      if (!is.null(self$`counterparty`)) {
        CustomerCommunicationObject[["counterparty"]] <-
          self$`counterparty`
      }
      if (!is.null(self$`direction`)) {
        CustomerCommunicationObject[["direction"]] <-
          self$extractSimpleType(self$`direction`)
      }
      if (!is.null(self$`occurredAt`)) {
        CustomerCommunicationObject[["occurredAt"]] <-
          self$`occurredAt`
      }
      if (!is.null(self$`subject`)) {
        CustomerCommunicationObject[["subject"]] <-
          self$`subject`
      }
      if (!is.null(self$`tags`)) {
        CustomerCommunicationObject[["tags"]] <-
          self$extractSimpleType(self$`tags`)
      }
      return(CustomerCommunicationObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of CustomerCommunication
    #'
    #' @param input_json the JSON input
    #' @return the instance of CustomerCommunication
    ...
    #' @return CustomerCommunication in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to CustomerCommunication and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`channel`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CustomerCommunication: the required field `channel` is missing."))
      }
      ...
      if (!is.null(input_json$`contactId`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CustomerCommunication: the required field `contactId` is missing."))
      }
      # check the required field `direction`
      if (!is.null(input_json$`direction`)) {
        stopifnot(R6::is.R6(input_json$`direction`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for CustomerCommunication: the required field `direction` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of CustomerCommunication
    toString = function() {
      self$toJSONString()
    },
...
# CustomerCommunication$unlock()
#
## Below is an example to define the print function
# CustomerCommunication$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# CustomerCommunication$lock()
