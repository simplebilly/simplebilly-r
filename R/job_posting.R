...
JobPosting <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new JobPosting class.
    #'
    ...
    #'
    #' @return JobPosting as a base R list.
    #' @examples
    #' # convert array of JobPosting (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert JobPosting to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      JobPostingObject <- list()
      if (!is.null(self$`currency`)) {
        JobPostingObject[["currency"]] <-
          self$`currency`
      }
      if (!is.null(self$`department`)) {
        JobPostingObject[["department"]] <-
          self$`department`
      }
      ...
      if (!is.null(self$`employmentType`)) {
        JobPostingObject[["employmentType"]] <-
          self$extractSimpleType(self$`employmentType`)
      }
      if (!is.null(self$`location`)) {
        JobPostingObject[["location"]] <-
          self$`location`
      }
      if (!is.null(self$`remote`)) {
        JobPostingObject[["remote"]] <-
          self$`remote`
      }
      if (!is.null(self$`requiredSkills`)) {
        JobPostingObject[["requiredSkills"]] <-
          self$extractSimpleType(self$`requiredSkills`)
      }
      if (!is.null(self$`requirements`)) {
        JobPostingObject[["requirements"]] <-
          self$`requirements`
      }
      if (!is.null(self$`salaryMax`)) {
        JobPostingObject[["salaryMax"]] <-
          self$`salaryMax`
      }
      if (!is.null(self$`salaryMin`)) {
        JobPostingObject[["salaryMin"]] <-
          self$`salaryMin`
      }
      if (!is.null(self$`status`)) {
        JobPostingObject[["status"]] <-
          self$extractSimpleType(self$`status`)
      }
      if (!is.null(self$`title`)) {
        JobPostingObject[["title"]] <-
          self$`title`
      }
      return(JobPostingObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of JobPosting
    #'
    #' @param input_json the JSON input
    #' @return the instance of JobPosting
    ...
    #' @return JobPosting in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to JobPosting and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`description`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for JobPosting: the required field `description` is missing."))
      }
      ...
      if (!is.null(input_json$`remote`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for JobPosting: the required field `remote` is missing."))
      }
      # check the required field `requiredSkills`
      if (!is.null(input_json$`requiredSkills`)) {
        stopifnot(R6::is.R6(input_json$`requiredSkills`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for JobPosting: the required field `requiredSkills` is missing."))
      }
      # check the required field `status`
      if (!is.null(input_json$`status`)) {
        stopifnot(R6::is.R6(input_json$`status`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for JobPosting: the required field `status` is missing."))
      }
      ...
      if (!is.null(input_json$`title`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for JobPosting: the required field `title` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of JobPosting
    toString = function() {
      self$toJSONString()
    },
...
# JobPosting$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
