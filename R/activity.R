...
Activity <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new Activity class.
    #'
    ...
    #'
    #' @return Activity as a base R list.
    #' @examples
    #' # convert array of Activity (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert Activity to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`assignedTo`)) {
        ActivityObject[["assignedTo"]] <-
          self$`assignedTo`
      }
      if (!is.null(self$`contactId`)) {
        ActivityObject[["contactId"]] <-
          self$`contactId`
      }
      ...
      if (!is.null(self$`dueDate`)) {
        ActivityObject[["dueDate"]] <-
          self$`dueDate`
      }
      if (!is.null(self$`reminderDate`)) {
        ActivityObject[["reminderDate"]] <-
          self$`reminderDate`
      }
    ...
    #' @description
    #' Deserialize JSON string into an instance of Activity
    #'
    ...
    #' @return Activity in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to Activity and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`activityType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Activity: the required field `activityType` is missing."))
      }
      # check the required field `status`
      if (!is.null(input_json$`status`)) {
        stopifnot(R6::is.R6(input_json$`status`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for Activity: the required field `status` is missing."))
      }
      ...
      if (!is.null(input_json$`subject`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Activity: the required field `subject` is missing."))
      }
    },
...
# Activity$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
