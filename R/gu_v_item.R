...
GuVItem <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new GuVItem class.
    #'
    #' @param account account
    #' @param amount amount
    #' @param name name
    #' @param ... Other optional arguments.
    initialize = function(`account`, `amount`, `name`, ...) {
      if (!missing(`account`)) {
    ...
    #'
    #' @return GuVItem as a base R list.
    #' @examples
    #' # convert array of GuVItem (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert GuVItem to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      GuVItemObject <- list()
      if (!is.null(self$`account`)) {
        GuVItemObject[["account"]] <-
          self$`account`
      }
      if (!is.null(self$`amount`)) {
        GuVItemObject[["amount"]] <-
          self$`amount`
      }
      ...
      return(GuVItemObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of GuVItem
    #'
    #' @param input_json the JSON input
    #' @return the instance of GuVItem
    ...
    #' @description
    #' Validate JSON input with respect to GuVItem and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`account`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GuVItem: the required field `account` is missing."))
      }
      ...
      if (!is.null(input_json$`amount`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GuVItem: the required field `amount` is missing."))
      }
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GuVItem: the required field `name` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of GuVItem
    toString = function() {
      self$toJSONString()
    },
...
# GuVItem$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
