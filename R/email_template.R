...
#' @docType class
#' @title EmailTemplate
#' @description EmailTemplate Class
...
EmailTemplate <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new EmailTemplate class.
    #'
    ...
    #'
    #' @return EmailTemplate as a base R list.
    #' @examples
    #' # convert array of EmailTemplate (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert EmailTemplate to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      EmailTemplateObject <- list()
      if (!is.null(self$`body`)) {
        EmailTemplateObject[["body"]] <-
          self$`body`
      }
      if (!is.null(self$`name`)) {
        EmailTemplateObject[["name"]] <-
          self$`name`
      }
      if (!is.null(self$`status`)) {
        EmailTemplateObject[["status"]] <-
          self$extractSimpleType(self$`status`)
      }
      if (!is.null(self$`subject`)) {
        EmailTemplateObject[["subject"]] <-
          self$`subject`
      }
      if (!is.null(self$`variables`)) {
        EmailTemplateObject[["variables"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of EmailTemplate
    #'
    #' @param input_json the JSON input
    #' @return the instance of EmailTemplate
    ...
    #' @return EmailTemplate in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to EmailTemplate and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`body`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for EmailTemplate: the required field `body` is missing."))
      }
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for EmailTemplate: the required field `name` is missing."))
      }
      # check the required field `status`
      if (!is.null(input_json$`status`)) {
        stopifnot(R6::is.R6(input_json$`status`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for EmailTemplate: the required field `status` is missing."))
      }
      ...
      if (!is.null(input_json$`subject`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for EmailTemplate: the required field `subject` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of EmailTemplate
    toString = function() {
      self$toJSONString()
    },
...
# EmailTemplate$unlock()
#
## Below is an example to define the print function
# EmailTemplate$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
