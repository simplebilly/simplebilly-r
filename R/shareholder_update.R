#' Create a new ShareholderUpdate
#'
#' @description
#' ShareholderUpdate Class
#'
#' @docType class
#' @title ShareholderUpdate
#' @description ShareholderUpdate Class
...
ShareholderUpdate <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ShareholderUpdate class.
    #'
    ...
    #'
    #' @return ShareholderUpdate as a base R list.
    #' @examples
    #' # convert array of ShareholderUpdate (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ShareholderUpdate to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ShareholderUpdateObject <- list()
      if (!is.null(self$`address`)) {
        ShareholderUpdateObject[["address"]] <-
          self$`address`
      }
      if (!is.null(self$`birthDate`)) {
        ShareholderUpdateObject[["birthDate"]] <-
          self$`birthDate`
      }
      if (!is.null(self$`email`)) {
        ShareholderUpdateObject[["email"]] <-
          self$`email`
      }
      if (!is.null(self$`firstName`)) {
        ShareholderUpdateObject[["firstName"]] <-
          self$`firstName`
      }
      if (!is.null(self$`lastName`)) {
        ShareholderUpdateObject[["lastName"]] <-
          self$`lastName`
      }
      if (!is.null(self$`shareNumber`)) {
        ShareholderUpdateObject[["shareNumber"]] <-
          self$`shareNumber`
      }
      if (!is.null(self$`shares`)) {
        ShareholderUpdateObject[["shares"]] <-
          self$`shares`
      }
      return(ShareholderUpdateObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of ShareholderUpdate
    #'
    #' @param input_json the JSON input
    #' @return the instance of ShareholderUpdate
    ...
    #' @return ShareholderUpdate in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ShareholderUpdate and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of ShareholderUpdate
    toString = function() {
      self$toJSONString()
    },
...
# ShareholderUpdate$unlock()
#
## Below is an example to define the print function
# ShareholderUpdate$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ShareholderUpdate$lock()
