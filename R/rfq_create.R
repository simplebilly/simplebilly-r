...
RfqCreate <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new RfqCreate class.
    #'
    ...
    #'
    #' @return RfqCreate as a base R list.
    #' @examples
    #' # convert array of RfqCreate (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert RfqCreate to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      RfqCreateObject <- list()
      if (!is.null(self$`currency`)) {
        RfqCreateObject[["currency"]] <-
          self$`currency`
      }
      if (!is.null(self$`lineItems`)) {
        RfqCreateObject[["lineItems"]] <-
          self$extractSimpleType(self$`lineItems`)
      }
      if (!is.null(self$`notes`)) {
        RfqCreateObject[["notes"]] <-
          self$`notes`
      }
      if (!is.null(self$`requestedDate`)) {
        RfqCreateObject[["requestedDate"]] <-
          self$`requestedDate`
      }
      if (!is.null(self$`responseDate`)) {
        RfqCreateObject[["responseDate"]] <-
          self$`responseDate`
      }
      if (!is.null(self$`rfqNumber`)) {
        RfqCreateObject[["rfqNumber"]] <-
          self$`rfqNumber`
      }
      if (!is.null(self$`status`)) {
        RfqCreateObject[["status"]] <-
          self$extractSimpleType(self$`status`)
      }
      if (!is.null(self$`supplierContactId`)) {
        RfqCreateObject[["supplierContactId"]] <-
          self$`supplierContactId`
      }
      if (!is.null(self$`supplierName`)) {
        RfqCreateObject[["supplierName"]] <-
          self$`supplierName`
      }
      return(RfqCreateObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of RfqCreate
    #'
    #' @param input_json the JSON input
    #' @return the instance of RfqCreate
    ...
    #' @return RfqCreate in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to RfqCreate and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`lineItems`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RfqCreate: the required field `lineItems` is missing."))
      }
      ...
      if (!is.null(input_json$`requestedDate`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RfqCreate: the required field `requestedDate` is missing."))
      }
      ...
      if (!is.null(input_json$`rfqNumber`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RfqCreate: the required field `rfqNumber` is missing."))
      }
      # check the required field `status`
      if (!is.null(input_json$`status`)) {
        stopifnot(R6::is.R6(input_json$`status`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for RfqCreate: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of RfqCreate
    toString = function() {
      self$toJSONString()
    },
...
# RfqCreate$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
