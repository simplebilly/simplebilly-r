...
#' @docType class
#' @title CouponCreate
#' @description CouponCreate Class
#' @format An \code{R6Class} generator object
#' @field code  character
#' @field description  character [optional]
#' @field discountType  \link{DiscountType}
#' @field discountValue  character
...
CouponCreate <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new CouponCreate class.
    #'
    #' @param code code
    ...
    initialize = function(`code`, `discountType`, `discountValue`, `description` = NULL, `expiresAt` = NULL, `isActive` = NULL, `isCombineable` = NULL, `maxDiscountAmount` = NULL, `maxUses` = NULL, `maxUsesPerCustomer` = NULL, `minOrderAmount` = NULL, `productIds` = NULL, `startsAt` = NULL, ...) {
      if (!missing(`code`)) {
    ...
    #'
    #' @return CouponCreate as a base R list.
    #' @examples
    #' # convert array of CouponCreate (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CouponCreate to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      CouponCreateObject <- list()
      if (!is.null(self$`code`)) {
        CouponCreateObject[["code"]] <-
          self$`code`
      }
      if (!is.null(self$`description`)) {
        CouponCreateObject[["description"]] <-
          self$`description`
      }
      if (!is.null(self$`discountType`)) {
        CouponCreateObject[["discountType"]] <-
          self$extractSimpleType(self$`discountType`)
      }
      if (!is.null(self$`discountValue`)) {
        CouponCreateObject[["discountValue"]] <-
          self$`discountValue`
      }
      if (!is.null(self$`expiresAt`)) {
        CouponCreateObject[["expiresAt"]] <-
          self$`expiresAt`
      }
      if (!is.null(self$`isActive`)) {
        CouponCreateObject[["isActive"]] <-
          self$`isActive`
      }
      if (!is.null(self$`isCombineable`)) {
        CouponCreateObject[["isCombineable"]] <-
          self$`isCombineable`
      }
      if (!is.null(self$`maxDiscountAmount`)) {
        CouponCreateObject[["maxDiscountAmount"]] <-
          self$`maxDiscountAmount`
      }
      if (!is.null(self$`maxUses`)) {
        CouponCreateObject[["maxUses"]] <-
          self$`maxUses`
      }
      if (!is.null(self$`maxUsesPerCustomer`)) {
        CouponCreateObject[["maxUsesPerCustomer"]] <-
          self$`maxUsesPerCustomer`
      }
      if (!is.null(self$`minOrderAmount`)) {
        CouponCreateObject[["minOrderAmount"]] <-
          self$`minOrderAmount`
      }
      if (!is.null(self$`productIds`)) {
        CouponCreateObject[["productIds"]] <-
          self$extractSimpleType(self$`productIds`)
      }
      if (!is.null(self$`startsAt`)) {
        CouponCreateObject[["startsAt"]] <-
          self$`startsAt`
      }
      return(CouponCreateObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of CouponCreate
    #'
    #' @param input_json the JSON input
    #' @return the instance of CouponCreate
    ...
    #' @return CouponCreate in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to CouponCreate and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CouponCreate: the required field `code` is missing."))
      }
      # check the required field `discountType`
      if (!is.null(input_json$`discountType`)) {
        stopifnot(R6::is.R6(input_json$`discountType`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for CouponCreate: the required field `discountType` is missing."))
      }
      # check the required field `discountValue`
      if (!is.null(input_json$`discountValue`)) {
        if (!(is.character(input_json$`discountValue`) && length(input_json$`discountValue`) == 1)) {
          stop(paste("Error! Invalid data for `discountValue`. Must be a string:", input_json$`discountValue`))
        }
      } else {
        stop(paste("The JSON input `", input, "` is invalid for CouponCreate: the required field `discountValue` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of CouponCreate
    toString = function() {
      self$toJSONString()
    },
    ...
    isValid = function() {
      ...
      # check if the required `discountType` is null
      if (is.null(self$`discountType`)) {
        return(FALSE)
      }

      # check if the required `discountValue` is null
      if (is.null(self$`discountValue`)) {
        return(FALSE)
      }
    ...
    getInvalidFields = function() {
      ...
      # check if the required `discountType` is null
      if (is.null(self$`discountType`)) {
        invalid_fields["discountType"] <- "Non-nullable required field `discountType` cannot be null."
      }

      # check if the required `discountValue` is null
      if (is.null(self$`discountValue`)) {
        invalid_fields["discountValue"] <- "Non-nullable required field `discountValue` cannot be null."
      }
...
# CouponCreate$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
