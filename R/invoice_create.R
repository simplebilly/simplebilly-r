...
#' @docType class
#' @title InvoiceCreate
#' @description InvoiceCreate Class
...
InvoiceCreate <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new InvoiceCreate class.
    #'
    ...
    #'
    #' @return InvoiceCreate as a base R list.
    #' @examples
    #' # convert array of InvoiceCreate (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Deserialize JSON string into an instance of InvoiceCreate
    #'
    #' @param input_json the JSON input
    #' @return the instance of InvoiceCreate
    ...
    #' @return InvoiceCreate in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to InvoiceCreate and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`currency`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for InvoiceCreate: the required field `currency` is missing."))
      }
      # check the required field `invoiceType`
      if (!is.null(input_json$`invoiceType`)) {
        stopifnot(R6::is.R6(input_json$`invoiceType`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for InvoiceCreate: the required field `invoiceType` is missing."))
      }
      ...
      if (!is.null(input_json$`issueDate`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for InvoiceCreate: the required field `issueDate` is missing."))
      }
      # check the required field `lineItems`
      if (!is.null(input_json$`lineItems`)) {
        stopifnot(R6::is.R6(input_json$`lineItems`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for InvoiceCreate: the required field `lineItems` is missing."))
      }
      # check the required field `status`
      if (!is.null(input_json$`status`)) {
        stopifnot(R6::is.R6(input_json$`status`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for InvoiceCreate: the required field `status` is missing."))
      }
      ...
      if (!is.null(input_json$`subtotal`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for InvoiceCreate: the required field `subtotal` is missing."))
      }
      ...
      if (!is.null(input_json$`totalAmount`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for InvoiceCreate: the required field `totalAmount` is missing."))
      }
      ...
      if (!is.null(input_json$`totalTax`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for InvoiceCreate: the required field `totalTax` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of InvoiceCreate
    toString = function() {
      self$toJSONString()
    },
...
# InvoiceCreate$unlock()
#
## Below is an example to define the print function
# InvoiceCreate$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
