...
#' @description ServiceAssignment Class
...
ServiceAssignment <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ServiceAssignment class.
    #'
    ...
    #'
    #' @return ServiceAssignment as a base R list.
    #' @examples
    #' # convert array of ServiceAssignment (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ServiceAssignment to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ServiceAssignmentObject <- list()
      if (!is.null(self$`employeeId`)) {
        ServiceAssignmentObject[["employeeId"]] <-
          self$`employeeId`
      }
      if (!is.null(self$`jobId`)) {
        ServiceAssignmentObject[["jobId"]] <-
          self$`jobId`
      }
      if (!is.null(self$`notes`)) {
        ServiceAssignmentObject[["notes"]] <-
          self$`notes`
      }
      if (!is.null(self$`scheduledDate`)) {
        ServiceAssignmentObject[["scheduledDate"]] <-
          self$`scheduledDate`
      }
      if (!is.null(self$`scheduledEnd`)) {
        ServiceAssignmentObject[["scheduledEnd"]] <-
          self$`scheduledEnd`
      }
      if (!is.null(self$`scheduledStart`)) {
        ServiceAssignmentObject[["scheduledStart"]] <-
          self$`scheduledStart`
      }
      if (!is.null(self$`status`)) {
        ServiceAssignmentObject[["status"]] <-
          self$extractSimpleType(self$`status`)
      }
      return(ServiceAssignmentObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of ServiceAssignment
    #'
    #' @param input_json the JSON input
    #' @return the instance of ServiceAssignment
    ...
    #' @return ServiceAssignment in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ServiceAssignment and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of ServiceAssignment
    toString = function() {
      self$toJSONString()
    },
...
# ServiceAssignment$unlock()
#
## Below is an example to define the print function
# ServiceAssignment$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ServiceAssignment$lock()
