...
PnLItem <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PnLItem class.
    #'
    ...
    #'
    #' @return PnLItem as a base R list.
    #' @examples
    #' # convert array of PnLItem (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PnLItem to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PnLItemObject <- list()
      if (!is.null(self$`account`)) {
        PnLItemObject[["account"]] <-
          self$`account`
      }
      if (!is.null(self$`account_name`)) {
        PnLItemObject[["account_name"]] <-
          self$`account_name`
      }
      if (!is.null(self$`amount`)) {
        PnLItemObject[["amount"]] <-
          self$`amount`
      }
      return(PnLItemObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PnLItem
    #'
    #' @param input_json the JSON input
    #' @return the instance of PnLItem
    ...
    #' @description
    #' Validate JSON input with respect to PnLItem and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`account`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PnLItem: the required field `account` is missing."))
      }
      ...
      if (!is.null(input_json$`account_name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PnLItem: the required field `account_name` is missing."))
      }
      ...
      if (!is.null(input_json$`amount`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PnLItem: the required field `amount` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PnLItem
    toString = function() {
      self$toJSONString()
    },
...
# PnLItem$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
