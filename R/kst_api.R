...
KstApi <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @param year 
    #' @param gewinn (optional) No description
    #' @param data_file (optional) name of the data file to save the result
    #' @param ... Other optional arguments
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return KstErgebnis
    KstApi = function(year, gewinn = NULL, data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$KstApiWithHttpInfo(year, gewinn, data_file = data_file, ..., .parse = .parse)
    ...
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return API response (KstErgebnis) with additional information such as HTTP status code, headers
    KstApiWithHttpInfo = function(year, gewinn = NULL, data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      if (!missing(`year`) && is.null(`year`)) {
        stop("Invalid value for `year` when calling KstApi$KstApi, `year` is not nullable")
      }


      query_params[["year"]] <- `year`

      query_params[["gewinn"]] <- `gewinn`

      local_var_url_path <- "/api/v1/bookkeeping/kst"
      # Bearer token
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$DeserializeResponse(local_var_resp, "KstErgebnis"),
          error = function(e) {
