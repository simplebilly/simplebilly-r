...
#'
#' @docType class
#' @title OssReport operations
#' @description OssReportApi
...
#' @export
OssReportApi <- R6::R6Class(
  "OssReportApi",
  public = list(
    api_client = NULL,

    #' @description
    #' Initialize a new OssReportApi.
    #'
    ...
    #'
    #' @return OssReport
    OssReportApi = function(data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$OssReportApiWithHttpInfo(data_file = data_file, ..., .parse = .parse)
    ...
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return API response (OssReport) with additional information such as HTTP status code, headers
    OssReportApiWithHttpInfo = function(data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      local_var_url_path <- "/api/v1/bookkeeping/oss"
      # Bearer token
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$DeserializeResponse(local_var_resp, "OssReport"),
          error = function(e) {
