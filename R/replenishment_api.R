...
#' @export
ReplenishmentApi <- R6::R6Class(
  "ReplenishmentApi",
  public = list(
    api_client = NULL,

    #' @description
    #' Initialize a new ReplenishmentApi.
    #'
    ...
    #' Create one draft stock transfer per (source → target) pair carrying all suggested product lines for that pair.
    #'
    #' @param target_warehouse_id (optional) Warehouse to be replenished. Defaults to the tenant's default warehouse.
    #' @param source_warehouse_id (optional) Restrict source warehouses to this id.
    #' @param data_file (optional) name of the data file to save the result
    #' @param ... Other optional arguments
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return AnyType
    ApplyReplenishments = function(target_warehouse_id = NULL, source_warehouse_id = NULL, data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$ApplyReplenishmentsWithHttpInfo(target_warehouse_id, source_warehouse_id, data_file = data_file, ..., .parse = .parse)
    ...
    ApplyReplenishmentsWithHttpInfo = function(target_warehouse_id = NULL, source_warehouse_id = NULL, data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      query_params[["targetWarehouseId"]] <- `target_warehouse_id`

      query_params[["sourceWarehouseId"]] <- `source_warehouse_id`

      local_var_url_path <- "/api/v1/replenishments/apply"
      # Bearer token
    ...
    GetReplenishments = function(target_warehouse_id = NULL, source_warehouse_id = NULL, data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$GetReplenishmentsWithHttpInfo(target_warehouse_id, source_warehouse_id, data_file = data_file, ..., .parse = .parse)
      if (local_var_response$status_code >= 200 && local_var_response$status_code <= 299) {
        return(local_var_response$content)
      } else if (local_var_response$status_code >= 300 && local_var_response$status_code <= 399) {
        return(local_var_response)
    ...
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return API response (ReplenishmentResponse) with additional information such as HTTP status code, headers
    GetReplenishmentsWithHttpInfo = function(target_warehouse_id = NULL, source_warehouse_id = NULL, data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      local_var_url_path <- "/api/v1/replenishments"
      # Bearer token
      if (!is.null(self$api_client$bearer_token)) {
        header_params["Authorization"] <- paste("Bearer", self$api_client$bearer_token, sep = " ")
      }
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$DeserializeResponse(local_var_resp, "ReplenishmentResponse"),
          error = function(e) {
            stop("Failed to deserialize response")
          }
        )
