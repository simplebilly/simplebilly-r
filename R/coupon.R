  ...
  public = list(
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Coupon: the required field `code` is missing."))
      }
      # check the required field `discountType`
      if (!is.null(input_json$`discountType`)) {
        stopifnot(R6::is.R6(input_json$`discountType`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for Coupon: the required field `discountType` is missing."))
      }
      ...
      if (!is.null(input_json$`discountValue`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Coupon: the required field `discountValue` is missing."))
      }
    },
