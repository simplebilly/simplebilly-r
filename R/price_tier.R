...
PriceTier <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PriceTier class.
    #'
    ...
    #'
    #' @return PriceTier as a base R list.
    #' @examples
    #' # convert array of PriceTier (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PriceTier to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PriceTierObject <- list()
      if (!is.null(self$`customerGroupId`)) {
        PriceTierObject[["customerGroupId"]] <-
          self$`customerGroupId`
      }
      if (!is.null(self$`minQuantity`)) {
        PriceTierObject[["minQuantity"]] <-
          self$`minQuantity`
      }
      if (!is.null(self$`productId`)) {
        PriceTierObject[["productId"]] <-
          self$`productId`
      }
      if (!is.null(self$`unitPrice`)) {
        PriceTierObject[["unitPrice"]] <-
          self$`unitPrice`
      }
      return(PriceTierObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PriceTier
    #'
    #' @param input_json the JSON input
    #' @return the instance of PriceTier
    ...
    #' @return PriceTier in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PriceTier and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`productId`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PriceTier: the required field `productId` is missing."))
      }
      ...
      if (!is.null(input_json$`unitPrice`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PriceTier: the required field `unitPrice` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PriceTier
    toString = function() {
      self$toJSONString()
    },
...
# PriceTier$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
