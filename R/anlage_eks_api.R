...
#'
#' @docType class
#' @title AnlageEks operations
#' @description AnlageEksApi
...
#' @export
AnlageEksApi <- R6::R6Class(
  "AnlageEksApi",
  public = list(
    api_client = NULL,

    #' @description
    #' Initialize a new AnlageEksApi.
    #'
    ...
    #'
    #' @return EksErgebnis
    EksApi = function(year, data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$EksApiWithHttpInfo(year, data_file = data_file, ..., .parse = .parse)
    ...
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return API response (EksErgebnis) with additional information such as HTTP status code, headers
    EksApiWithHttpInfo = function(year, data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      if (!missing(`year`) && is.null(`year`)) {
        stop("Invalid value for `year` when calling AnlageEksApi$EksApi, `year` is not nullable")
      }

      query_params[["year"]] <- `year`

      local_var_url_path <- "/api/v1/bookkeeping/eks"
      # Bearer token
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$DeserializeResponse(local_var_resp, "EksErgebnis"),
          error = function(e) {
