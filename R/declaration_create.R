#' Create a new DeclarationCreate
#'
#' @description
#' DeclarationCreate Class
#'
#' @docType class
#' @title DeclarationCreate
#' @description DeclarationCreate Class
...
DeclarationCreate <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new DeclarationCreate class.
    #'
    ...
    #'
    #' @return DeclarationCreate as a base R list.
    #' @examples
    #' # convert array of DeclarationCreate (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert DeclarationCreate to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      DeclarationCreateObject <- list()
      if (!is.null(self$`declarationType`)) {
        DeclarationCreateObject[["declarationType"]] <-
          self$extractSimpleType(self$`declarationType`)
      }
      if (!is.null(self$`isCurrent`)) {
        DeclarationCreateObject[["isCurrent"]] <-
          self$`isCurrent`
      }
      if (!is.null(self$`text`)) {
        DeclarationCreateObject[["text"]] <-
          self$`text`
      }
      if (!is.null(self$`validFrom`)) {
        DeclarationCreateObject[["validFrom"]] <-
          self$`validFrom`
      }
      if (!is.null(self$`version`)) {
        DeclarationCreateObject[["version"]] <-
          self$`version`
      }
      return(DeclarationCreateObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of DeclarationCreate
    #'
    #' @param input_json the JSON input
    #' @return the instance of DeclarationCreate
    ...
    #' @return DeclarationCreate in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to DeclarationCreate and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of DeclarationCreate
    toString = function() {
      self$toJSONString()
    },
...
# DeclarationCreate$unlock()
#
## Below is an example to define the print function
# DeclarationCreate$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# DeclarationCreate$lock()
