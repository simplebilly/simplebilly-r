...
SearchApi <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return AnyType
    GlobalSearch = function(q, data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$GlobalSearchWithHttpInfo(q, data_file = data_file, ..., .parse = .parse)
    ...
    GlobalSearchWithHttpInfo = function(q, data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      if (!missing(`q`) && is.null(`q`)) {
        stop("Invalid value for `q` when calling SearchApi$GlobalSearch, `q` is not nullable")
      }

      query_params[["q"]] <- `q`

      local_var_url_path <- "/api/v1/search"
      # Bearer token
    ...
    #'
    #' @return AnyType
    MyPermissions = function(data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$MyPermissionsWithHttpInfo(data_file = data_file, ..., .parse = .parse)
      if (local_var_response$status_code >= 200 && local_var_response$status_code <= 299) {
        return(local_var_response$content)
      } else if (local_var_response$status_code >= 300 && local_var_response$status_code <= 399) {
        return(local_var_response)
    ...
    #'
    #' @return API response (AnyType) with additional information such as HTTP status code, headers
    MyPermissionsWithHttpInfo = function(data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      local_var_url_path <- "/api/v1/me/permissions"
      # Bearer token
      if (!is.null(self$api_client$bearer_token)) {
        header_params["Authorization"] <- paste("Bearer", self$api_client$bearer_token, sep = " ")
      }
