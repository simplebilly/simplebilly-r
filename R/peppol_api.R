...
#'
#' @docType class
#' @title Peppol operations
#' @description PeppolApi
...
#' @export
PeppolApi <- R6::R6Class(
  "PeppolApi",
  public = list(
    api_client = NULL,

    #' @description
    #' Initialize a new PeppolApi.
    #'
    ...
    PeppolApi = function(id, data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$PeppolApiWithHttpInfo(id, data_file = data_file, ..., .parse = .parse)
    ...
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return API response (PeppolResponse) with additional information such as HTTP status code, headers
    PeppolApiWithHttpInfo = function(id, data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      if (!missing(`id`) && is.null(`id`)) {
        stop("Invalid value for `id` when calling PeppolApi$PeppolApi, `id` is not nullable")
      }

      local_var_url_path <- "/api/v1/invoices/{id}/peppol"
      if (!missing(`id`)) {
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$DeserializeResponse(local_var_resp, "PeppolResponse"),
          error = function(e) {
