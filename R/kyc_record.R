...
KycRecord <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new KycRecord class.
    #'
    ...
    #'
    #' @return KycRecord as a base R list.
    #' @examples
    #' # convert array of KycRecord (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert KycRecord to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      KycRecordObject <- list()
      if (!is.null(self$`customerId`)) {
        KycRecordObject[["customerId"]] <-
          self$`customerId`
      }
      if (!is.null(self$`customerName`)) {
        KycRecordObject[["customerName"]] <-
          self$`customerName`
      }
      if (!is.null(self$`kycDate`)) {
        KycRecordObject[["kycDate"]] <-
          self$`kycDate`
      }
      if (!is.null(self$`notes`)) {
        KycRecordObject[["notes"]] <-
          self$`notes`
      }
      if (!is.null(self$`retentionUntil`)) {
        KycRecordObject[["retentionUntil"]] <-
          self$`retentionUntil`
      }
      if (!is.null(self$`riskAssessment`)) {
        KycRecordObject[["riskAssessment"]] <-
          self$`riskAssessment`
      }
      return(KycRecordObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of KycRecord
    #'
    #' @param input_json the JSON input
    #' @return the instance of KycRecord
    ...
    #' @return KycRecord in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to KycRecord and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of KycRecord
    toString = function() {
      self$toJSONString()
    },
...
# KycRecord$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
