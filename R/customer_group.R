...
#' @docType class
#' @title CustomerGroup
#' @description CustomerGroup Class
...
CustomerGroup <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new CustomerGroup class.
    #'
    ...
    #'
    #' @return CustomerGroup as a base R list.
    #' @examples
    #' # convert array of CustomerGroup (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CustomerGroup to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      CustomerGroupObject <- list()
      if (!is.null(self$`description`)) {
        CustomerGroupObject[["description"]] <-
          self$`description`
      }
      if (!is.null(self$`memberIds`)) {
        CustomerGroupObject[["memberIds"]] <-
          self$`memberIds`
      }
      if (!is.null(self$`membershipFilter`)) {
        CustomerGroupObject[["membershipFilter"]] <-
          self$`membershipFilter`
      }
      if (!is.null(self$`name`)) {
        CustomerGroupObject[["name"]] <-
          self$`name`
      }
    ...
    #' @description
    #' Deserialize JSON string into an instance of CustomerGroup
    #'
    #' @param input_json the JSON input
    #' @return the instance of CustomerGroup
    ...
    #' @return CustomerGroup in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to CustomerGroup and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CustomerGroup: the required field `name` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of CustomerGroup
    toString = function() {
      self$toJSONString()
    },
...
# CustomerGroup$unlock()
#
## Below is an example to define the print function
# CustomerGroup$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
