...
#' @docType class
#' @title VoucherCreate
#' @description VoucherCreate Class
...
VoucherCreate <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new VoucherCreate class.
    #'
    ...
    #'
    #' @return VoucherCreate as a base R list.
    #' @examples
    #' # convert array of VoucherCreate (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Deserialize JSON string into an instance of VoucherCreate
    #'
    #' @param input_json the JSON input
    #' @return the instance of VoucherCreate
    ...
    #' @return VoucherCreate in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to VoucherCreate and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`currency`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for VoucherCreate: the required field `currency` is missing."))
      }
      ...
      if (!is.null(input_json$`voucherDate`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for VoucherCreate: the required field `voucherDate` is missing."))
      }
      # check the required field `voucherStatus`
      if (!is.null(input_json$`voucherStatus`)) {
        stopifnot(R6::is.R6(input_json$`voucherStatus`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for VoucherCreate: the required field `voucherStatus` is missing."))
      }
      # check the required field `voucherType`
      if (!is.null(input_json$`voucherType`)) {
        stopifnot(R6::is.R6(input_json$`voucherType`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for VoucherCreate: the required field `voucherType` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of VoucherCreate
    toString = function() {
      self$toJSONString()
    },
...
# VoucherCreate$unlock()
#
## Below is an example to define the print function
# VoucherCreate$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
