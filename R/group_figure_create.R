#' Create a new GroupFigureCreate
#'
#' @description
#' GroupFigureCreate Class
#'
#' @docType class
#' @title GroupFigureCreate
#' @description GroupFigureCreate Class
...
GroupFigureCreate <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new GroupFigureCreate class.
    #'
    ...
    #'
    #' @return GroupFigureCreate as a base R list.
    #' @examples
    #' # convert array of GroupFigureCreate (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert GroupFigureCreate to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      GroupFigureCreateObject <- list()
      if (!is.null(self$`bilanzsumme`)) {
        GroupFigureCreateObject[["bilanzsumme"]] <-
          self$`bilanzsumme`
      }
      if (!is.null(self$`exemptionClaimed`)) {
        GroupFigureCreateObject[["exemptionClaimed"]] <-
          self$`exemptionClaimed`
      }
      if (!is.null(self$`mitarbeiter`)) {
        GroupFigureCreateObject[["mitarbeiter"]] <-
          self$`mitarbeiter`
      }
      if (!is.null(self$`nettoUmsatz`)) {
        GroupFigureCreateObject[["nettoUmsatz"]] <-
          self$`nettoUmsatz`
      }
      if (!is.null(self$`parentName`)) {
        GroupFigureCreateObject[["parentName"]] <-
          self$`parentName`
      }
      if (!is.null(self$`parentSitus`)) {
        GroupFigureCreateObject[["parentSitus"]] <-
          self$`parentSitus`
      }
      return(GroupFigureCreateObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of GroupFigureCreate
    #'
    #' @param input_json the JSON input
    #' @return the instance of GroupFigureCreate
    ...
    #' @return GroupFigureCreate in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to GroupFigureCreate and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of GroupFigureCreate
    toString = function() {
      self$toJSONString()
    },
...
# GroupFigureCreate$unlock()
#
## Below is an example to define the print function
# GroupFigureCreate$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# GroupFigureCreate$lock()
