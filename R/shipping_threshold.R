#' Create a new ShippingThreshold
#'
#' @description
#' ShippingThreshold Class
#'
#' @docType class
#' @title ShippingThreshold
#' @description ShippingThreshold Class
...
ShippingThreshold <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ShippingThreshold class.
    #'
    #' @param name name
    ...
    #'
    #' @return ShippingThreshold as a base R list.
    #' @examples
    #' # convert array of ShippingThreshold (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ShippingThreshold to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ShippingThresholdObject <- list()
      if (!is.null(self$`isActive`)) {
        ShippingThresholdObject[["isActive"]] <-
          self$`isActive`
      }
      if (!is.null(self$`maxSellable`)) {
        ShippingThresholdObject[["maxSellable"]] <-
          self$`maxSellable`
      }
      if (!is.null(self$`name`)) {
        ShippingThresholdObject[["name"]] <-
          self$`name`
      }
      if (!is.null(self$`notes`)) {
        ShippingThresholdObject[["notes"]] <-
          self$`notes`
      }
      if (!is.null(self$`productId`)) {
        ShippingThresholdObject[["productId"]] <-
          self$`productId`
      }
      if (!is.null(self$`reserveStock`)) {
        ShippingThresholdObject[["reserveStock"]] <-
          self$`reserveStock`
      }
      if (!is.null(self$`warehouseId`)) {
        ShippingThresholdObject[["warehouseId"]] <-
          self$`warehouseId`
      }
      return(ShippingThresholdObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of ShippingThreshold
    #'
    #' @param input_json the JSON input
    #' @return the instance of ShippingThreshold
    ...
    #' @return ShippingThreshold in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ShippingThreshold and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ShippingThreshold: the required field `name` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of ShippingThreshold
    toString = function() {
      self$toJSONString()
    },
...
# ShippingThreshold$unlock()
#
## Below is an example to define the print function
# ShippingThreshold$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ShippingThreshold$lock()
