...
#'
#' @docType class
#' @title Suitability operations
#' @description SuitabilityApi
...
#' @export
SuitabilityApi <- R6::R6Class(
  "SuitabilityApi",
  public = list(
    api_client = NULL,

    #' @description
    #' Initialize a new SuitabilityApi.
    #'
    ...
    #' 
    #'
    #' @param suitability_request 
    #' @param data_file (optional) name of the data file to save the result
    #' @param ... Other optional arguments
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return SuitabilityResult
    ShippingSuitabilityApi = function(suitability_request, data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$ShippingSuitabilityApiWithHttpInfo(suitability_request, data_file = data_file, ..., .parse = .parse)
    ...
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return API response (SuitabilityResult) with additional information such as HTTP status code, headers
    ShippingSuitabilityApiWithHttpInfo = function(suitability_request, data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      if (missing(`suitability_request`)) {
        stop("Missing required parameter `suitability_request`.")
      }

      if (!missing(`suitability_request`) && is.null(`suitability_request`)) {
        stop("Invalid value for `suitability_request` when calling SuitabilityApi$ShippingSuitabilityApi, `suitability_request` is not nullable")
      }

      if (!is.null(`suitability_request`)) {
        local_var_body <- `suitability_request`$toJSONString()
      } else {
        local_var_body <- NULL
      }

      local_var_url_path <- "/api/v1/shipping/suitability"
      # Bearer token
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$DeserializeResponse(local_var_resp, "SuitabilityResult"),
          error = function(e) {
