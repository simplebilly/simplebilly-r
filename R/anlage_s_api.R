...
#'
#' @docType class
#' @title AnlageS operations
#' @description AnlageSApi
...
#' @export
AnlageSApi <- R6::R6Class(
  "AnlageSApi",
  public = list(
    api_client = NULL,

    #' @description
    #' Initialize a new AnlageSApi.
    #'
    ...
    AnlageSApi = function(year, data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$AnlageSApiWithHttpInfo(year, data_file = data_file, ..., .parse = .parse)
    ...
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return API response (AnlageSErgebnis) with additional information such as HTTP status code, headers
    AnlageSApiWithHttpInfo = function(year, data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      if (!missing(`year`) && is.null(`year`)) {
        stop("Invalid value for `year` when calling AnlageSApi$AnlageSApi, `year` is not nullable")
      }

      query_params[["year"]] <- `year`

      local_var_url_path <- "/api/v1/bookkeeping/anlage-s"
      # Bearer token
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$DeserializeResponse(local_var_resp, "AnlageSErgebnis"),
          error = function(e) {
