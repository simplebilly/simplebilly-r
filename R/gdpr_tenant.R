...
GdprTenant <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new GdprTenant class.
    #'
    #' @param name name
    #' @param role role
    #' @param tenantId tenantId
    #' @param ... Other optional arguments.
    initialize = function(`name`, `role`, `tenantId`, ...) {
      if (!missing(`name`)) {
    ...
    #'
    #' @return GdprTenant as a base R list.
    #' @examples
    #' # convert array of GdprTenant (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert GdprTenant to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      GdprTenantObject <- list()
      if (!is.null(self$`name`)) {
        GdprTenantObject[["name"]] <-
          self$`name`
      }
      if (!is.null(self$`role`)) {
        GdprTenantObject[["role"]] <-
          self$`role`
      }
      if (!is.null(self$`tenantId`)) {
        GdprTenantObject[["tenantId"]] <-
          self$`tenantId`
      }
      return(GdprTenantObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of GdprTenant
    #'
    #' @param input_json the JSON input
    #' @return the instance of GdprTenant
    ...
    #' @return GdprTenant in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to GdprTenant and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GdprTenant: the required field `name` is missing."))
      }
      ...
      if (!is.null(input_json$`role`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GdprTenant: the required field `role` is missing."))
      }
      ...
      if (!is.null(input_json$`tenantId`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GdprTenant: the required field `tenantId` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of GdprTenant
    toString = function() {
      self$toJSONString()
    },
...
# GdprTenant$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
