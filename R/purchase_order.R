...
PurchaseOrder <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PurchaseOrder class.
    #'
    ...
    #'
    #' @return PurchaseOrder as a base R list.
    #' @examples
    #' # convert array of PurchaseOrder (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Deserialize JSON string into an instance of PurchaseOrder
    #'
    #' @param input_json the JSON input
    #' @return the instance of PurchaseOrder
    ...
    #' @return PurchaseOrder in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PurchaseOrder and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`orderDate`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PurchaseOrder: the required field `orderDate` is missing."))
      }
      ...
      if (!is.null(input_json$`poNumber`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PurchaseOrder: the required field `poNumber` is missing."))
      }
      # check the required field `status`
      if (!is.null(input_json$`status`)) {
        stopifnot(R6::is.R6(input_json$`status`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for PurchaseOrder: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PurchaseOrder
    toString = function() {
      self$toJSONString()
    },
...
# PurchaseOrder$unlock()
#
## Below is an example to define the print function
# PurchaseOrder$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
