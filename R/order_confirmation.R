#' Create a new OrderConfirmation
#'
#' @description
...
#' @description OrderConfirmation Class
...
OrderConfirmation <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new OrderConfirmation class.
    #'
    ...
    initialize = function(`currency`, `voucherDate`, `voucherStatus`, `address` = NULL, `confirmationNumber` = NULL, `contactId` = NULL, `contactName` = NULL, `files` = NULL, `introduction` = NULL, `lineItems` = NULL, `precedingSalesVoucherId` = NULL, `precedingSalesVoucherType` = NULL, `remark` = NULL, `subtotal` = NULL, `taxCondition` = NULL, `title` = NULL, `totalAmount` = NULL, `totalTax` = NULL, ...) {
      if (!missing(`currency`)) {
    ...
    #'
    #' @return OrderConfirmation as a base R list.
    #' @examples
    #' # convert array of OrderConfirmation (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert OrderConfirmation to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      OrderConfirmationObject <- list()
      if (!is.null(self$`address`)) {
        OrderConfirmationObject[["address"]] <-
          self$extractSimpleType(self$`address`)
      }
      if (!is.null(self$`confirmationNumber`)) {
        OrderConfirmationObject[["confirmationNumber"]] <-
          self$`confirmationNumber`
      }
      if (!is.null(self$`contactId`)) {
        OrderConfirmationObject[["contactId"]] <-
          self$`contactId`
      }
      if (!is.null(self$`contactName`)) {
        OrderConfirmationObject[["contactName"]] <-
          self$`contactName`
      }
      if (!is.null(self$`currency`)) {
        OrderConfirmationObject[["currency"]] <-
          self$`currency`
      }
      if (!is.null(self$`files`)) {
        OrderConfirmationObject[["files"]] <-
          self$extractSimpleType(self$`files`)
      }
      if (!is.null(self$`introduction`)) {
        OrderConfirmationObject[["introduction"]] <-
          self$`introduction`
      }
      if (!is.null(self$`lineItems`)) {
        OrderConfirmationObject[["lineItems"]] <-
          self$extractSimpleType(self$`lineItems`)
      }
      if (!is.null(self$`precedingSalesVoucherId`)) {
        OrderConfirmationObject[["precedingSalesVoucherId"]] <-
          self$`precedingSalesVoucherId`
      }
      if (!is.null(self$`precedingSalesVoucherType`)) {
        OrderConfirmationObject[["precedingSalesVoucherType"]] <-
          self$extractSimpleType(self$`precedingSalesVoucherType`)
      }
      if (!is.null(self$`remark`)) {
        OrderConfirmationObject[["remark"]] <-
          self$`remark`
      }
      if (!is.null(self$`subtotal`)) {
        OrderConfirmationObject[["subtotal"]] <-
          self$`subtotal`
      }
      if (!is.null(self$`taxCondition`)) {
        OrderConfirmationObject[["taxCondition"]] <-
          self$`taxCondition`
      }
      if (!is.null(self$`title`)) {
        OrderConfirmationObject[["title"]] <-
          self$`title`
      }
      if (!is.null(self$`totalAmount`)) {
        OrderConfirmationObject[["totalAmount"]] <-
          self$`totalAmount`
      }
      if (!is.null(self$`totalTax`)) {
        OrderConfirmationObject[["totalTax"]] <-
          self$`totalTax`
      }
      if (!is.null(self$`voucherDate`)) {
        OrderConfirmationObject[["voucherDate"]] <-
          self$`voucherDate`
      }
      if (!is.null(self$`voucherStatus`)) {
        OrderConfirmationObject[["voucherStatus"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of OrderConfirmation
    #'
    #' @param input_json the JSON input
    #' @return the instance of OrderConfirmation
    ...
    #' @return OrderConfirmation in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to OrderConfirmation and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`currency`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrderConfirmation: the required field `currency` is missing."))
      }
      ...
      if (!is.null(input_json$`voucherDate`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrderConfirmation: the required field `voucherDate` is missing."))
      }
      # check the required field `voucherStatus`
      if (!is.null(input_json$`voucherStatus`)) {
        stopifnot(R6::is.R6(input_json$`voucherStatus`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for OrderConfirmation: the required field `voucherStatus` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of OrderConfirmation
    toString = function() {
      self$toJSONString()
    },
...
# OrderConfirmation$unlock()
#
## Below is an example to define the print function
# OrderConfirmation$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# OrderConfirmation$lock()
