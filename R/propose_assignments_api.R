...
#'
#' @docType class
#' @title ProposeAssignments operations
#' @description ProposeAssignmentsApi
...
#' @export
ProposeAssignmentsApi <- R6::R6Class(
  "ProposeAssignmentsApi",
  public = list(
    api_client = NULL,

    #' @description
    #' Initialize a new ProposeAssignmentsApi.
    #'
    ...
    #' 
    #'
    #' @param min_confidence (optional) No description
    ...
    ProposeAssignmentsApi = function(min_confidence = NULL, customer_id = NULL, data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$ProposeAssignmentsApiWithHttpInfo(min_confidence, customer_id, data_file = data_file, ..., .parse = .parse)
    ...
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return API response (array[ProposedAssignment]) with additional information such as HTTP status code, headers
    ProposeAssignmentsApiWithHttpInfo = function(min_confidence = NULL, customer_id = NULL, data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      if (!missing(`min_confidence`) && is.null(`min_confidence`)) {
        stop("Invalid value for `min_confidence` when calling ProposeAssignmentsApi$ProposeAssignmentsApi, `min_confidence` is not nullable")
      }
      ...
      local_var_url_path <- "/api/v1/bookkeeping/propose-assignments"
      # Bearer token
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$DeserializeResponse(local_var_resp, "array[ProposedAssignment]"),
          error = function(e) {
